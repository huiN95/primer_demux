#!/usr/bin/env bash
set -euo pipefail
shopt -s nullglob

PRIMER="/mnt/data_7t/adam/adapter_bcs/primer40_v2.fasta"
IN_DIR="/mnt/data_7t/adam/primer_demux/barcode_filter_reads"
OUT_BASE="/mnt/data_7t/adam/primer_demux/primer_demuxed"   # Outputs are placed in the same parent directory

MAX_DISTANCE=2
TAIL_CUTOFF=0
KEEP_PRIMER="--keep_primer"
OUTPUT_FORMAT="fq"

# Optional: use a done marker file to skip completed samples
# DONE_FILE=".done"

# Supports fastq / fq / fastq.gz / fq.gz
files=( "$IN_DIR"/*.fastq "$IN_DIR"/*.fq "$IN_DIR"/*.fastq.gz "$IN_DIR"/*.fq.gz )

for f in "${files[@]}"; do
  # Filename (excluding path)
  base="$(basename "$f")"
  # Remove compression suffix
  base="${base%.gz}"
  # Remove fastq/fq suffix
  name="${base%.fastq}"
  name="${name%.fq}"

  out_dir="$OUT_BASE/$name"
  log_dir="$out_dir"   # Keep consistency since log_folder and output_folder were originally the same

  mkdir -p "$out_dir"

  echo "==> Processing: $f"
  cargo run --release -- \
    --primer "$PRIMER" \
    --max_distance "$MAX_DISTANCE" \
    --input_file "$f" \
    --output_folder "$out_dir" \
    --log_folder "$log_dir" \
    --tail_cutoff "$TAIL_CUTOFF" \
    $KEEP_PRIMER \
    --output_format "$OUTPUT_FORMAT"
done
